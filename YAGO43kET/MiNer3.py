...
class MiNer3(nn.Module):
    def __init__(self, hidden_dim, num_nodes, num_rels, num_types, alpha=0.9, beta=0.6):
        super(MiNer3, self).__init__()
        ...
        self.MTILayer = MTILayer(num_nodes, self.embedding_dim, num_types, self.beta)
    ...
    def forward(self, blocks):
        src0 = torch.index_select(self.entity, 0, blocks[0].srcdata['id'])
        dst0 = torch.index_select(self.entity, 0, blocks[0].dstdata['id'])
        etype0 = blocks[0].edata['etype']
        relations0 = torch.index_select(self.relation, 0, etype0 % self.num_rels)
        relations0[etype0 >= self.num_rels] = relations0[etype0 >= self.num_rels] * -1

        src1 = self.MulLayer(blocks[0], src0, dst0, relations0)
        dst1 = torch.index_select(self.entity, 0, blocks[1].dstdata['id'])
        etype1 = blocks[1].edata['etype']
        ...
        agg = self.AGGLayer(blocks[1], src1, relations1)
        src2 = torch.index_select(self.entity, 0, blocks[2].srcdata['id'])
        etype2 = blocks[2].edata['etype']
        ...
        output2 = self.MTILayer(blocks[2], src2, relations2, agg).sigmoid()
        return output2


class MTILayer(nn.Module):
    def __init__(self, num_nodes, embedding_dim, num_types, beta):
