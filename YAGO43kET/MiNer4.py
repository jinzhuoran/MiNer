...
class MiNer4(nn.Module):
    ...
    def forward(self, blocks):
        ...
        dst1 = torch.index_select(self.entity, 0, blocks[0].dstdata['id'])
        ...
        src1 = self.MulLayer(blocks[0], src1, dst1, relations1)
        dst1 = torch.index_select(self.entity, 0, blocks[1].dstdata['id'])
        etype1 = blocks[1].edata['etype']
        relations1 = torch.index_select(self.relation, 0, etype1 % self.num_rels)
        relations1[etype1 >= self.num_rels] = relations1[etype1 >= self.num_rels] * -1

        src1 = self.MulLayer(blocks[1], src1, dst1, relations1)
        etype1 = blocks[2].edata['etype']
        relations1 = torch.index_select(self.relation, 0, etype1 % self.num_rels)
        relations1[etype1 >= self.num_rels] = relations1[etype1 >= self.num_rels] * -1
        agg = self.AGGLayer(blocks[2], src1, relations1)

        src2 = torch.index_select(self.entity, 0, blocks[3].srcdata['id'])
        etype2 = blocks[3].edata['etype']
        ...
        output2 = self.MTILayer(blocks[3], src2, relations2, agg).sigmoid()
        return output2
