...
def load_e2t(path, e2id, t2id):
    e2t = [[]] * (len(e2id) + len(t2id))
    for i in range(len(e2id) + len(t2id)):
        e2t[i] = []
    max_len = 0
    with open(path, encoding='utf-8') as r:
        for line in r:
            e, t = line.strip().split('\t')
            e2t[e2id[e]].append(t2id[t])
            max_len = max(max_len, len(e2t[e2id[e]]))
    for tid in t2id.values():
        e2t[tid + len(e2id)].append(tid)
    for i in range(len(e2t)):
        e2t[i] = e2t[i] + [len(t2id)] * (max_len - len(e2t[i]))
    return e2t
